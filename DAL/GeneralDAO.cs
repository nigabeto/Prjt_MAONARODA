    ...
    public class GeneralDAO : PostContext
    {
        public List<PostDTO> GetSliderPosts()
        {
            ...
            foreach (var item in list)
            {
                ...
                dto.CommentCount = db.Comments
                    .Where(x => x.isDeleted == false && x.PostID == item.postID && x.isApproved == true).Count();
        ...
        public PostDTO GetPostDetail(int ID)
        {
            ...
            dto.CommentCount = db.Comments.Where(x => x.isDeleted == false && x.PostID == ID && x.isApproved == true)
                .Count();
            List<Comment> comments = db.Comments
                .Where(x => x.isDeleted == false && x.PostID == ID && x.isApproved == true).ToList();
        ...
        public List<PostDTO> GetSearchPost(string searchText)
        {
            ...
            var list = (from p in db.Posts.Where(x =>
                        x.isDeleted == false &&
                        (x.Title.Contains(searchText) || x.PostContent.Contains(searchText)))
                    join c in db.Categories on p.CategoryID equals c.ID
                    select new
                    {
                        postID = p.ID,
        ...
        public List<PostDTO> GetMostViewedPosts()
        {
            ...
            var list = (from p in db.Posts.Where(x => x.isDeleted == false)
                ...
                ).OrderByDescending(x => x.viewcout).Take(5).ToList();
            foreach (var item in list)
            {
                PostDTO dto = new PostDTO();
                dto.ID = item.postID;
